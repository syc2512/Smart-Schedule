...
class UserConfigsDAO:
    """用户配置 DAO。负责 API_KEY 加密 / 课程 JSON 序列化。"""
    ...
    def _row_to_dict(self, row: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if enc:
            try:
                result["api_key"] = self.crypto.decrypt_api_key(enc)
            except Exception:
                result["api_key"] = ""  # 解密失败返回空，不抛异常给前端
        else:
            result["api_key"] = ""
        # 课程 JSON 解析
    ...
    def find_by_user_id(self, user_id: int) -> Optional[Dict[str, Any]]:
        """返回完整配置（含解密后的 API_KEY）。"""
        sql = ("SELECT user_id, api_url, model_name, api_key_encrypted, "
    ...
    def find_public_by_user_id(self, user_id: int) -> Optional[Dict[str, Any]]:
        """返回配置（不含 API_KEY 明文，但返回 has_api_key 布尔）。

        适用于前端展示用，避免明文 API_KEY 走网络。
        """
        full = self.find_by_user_id(user_id)
        if full is None:
            return None
        has_key = bool(full.get("api_key"))
        # API_KEY 用掩码形式返回（前 4 + ... + 后 4），不返回明文
        full["api_key_masked"] = self._mask_api_key(full["api_key"]) if has_key else ""
        full["has_api_key"] = has_key
        full["api_key"] = ""  # 永不下发明文
        return full
    ...
    def upsert(self, user_id: int, data: Dict[str, Any]) -> None:
        ...
        :param data:    配置字典，可包含字段：
                        api_url, model_name, api_key, courses,
                        semester_start, total_weeks, extra_settings
        """
        api_url = (data.get("api_url") or "").strip()[:512]
        model_name = (data.get("model_name") or "").strip()[:128]
        # API_KEY 加密
        api_key_plain = (data.get("api_key") or "")
        if api_key_plain:
            api_key_enc = self.crypto.encrypt_api_key(api_key_plain)
        else:
            api_key_enc = None
        # 课程数据序列化
        ...
        sql = (
            ...
            "    (user_id, api_url, model_name, api_key_encrypted, "
            "     courses_json, semester_start, total_weeks, extra_settings) "
            "VALUES (%s, %s, %s, %s, %s, %s, %s, %s) "
            "ON DUPLICATE KEY UPDATE "
            "    api_url = VALUES(api_url), "
            "    model_name = VALUES(model_name), "
            "    api_key_encrypted = IF(VALUES(api_key_encrypted) IS NULL, "
        ...
        params = (user_id, api_url, model_name, api_key_enc,
                  courses_json, semester_start, total_weeks, extra_json)
        try:
            with self.pool.get_conn() as conn:
                with conn.cursor() as cur:
    ...
    def update_partial(self, user_id: int, data: Dict[str, Any]) -> None:
        ...
        特殊处理：
        - api_key 为空字符串: 不修改（保留原值）
        - api_key 为非空字符串: 加密后更新
        ...
        merged = {
            "api_url": data.get("api_url", current.get("api_url", "")),
            "model_name": data.get("model_name", current.get("model_name", "")),
        ...
        if "api_key" in data:
            ...
            if new_key:
                merged["api_key"] = new_key  # 有新值
            else:
                # 空字符串 = 不修改，沿用当前明文（重新加密）
...
if __name__ == "__main__":
    print("DB_DRIVER =", _DRIVER or "(未安装)")
    if _DRIVER is None:
        print("请执行: pip install PyMySQL")
    else:
        ok = check_connection()
        print("数据库连通性:", "OK" if ok else "FAILED")
