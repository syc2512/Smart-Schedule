...
# 韧性大模型客户端（超时 / 重试 / 熔断 / 限流 / 日志），见 llm_client.py
from llm_client import get_llm_client, load_config, LlmError
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle_ai_parse(self):
        ...
        try:
            content = client.chat(text, system_prompt=PARSE_SYSTEM_PROMPT)
        except LlmError as e:
            # 错误码 -> HTTP 状态映射：
            #   未配置 Key / 熔断中 -> 503（服务端暂不可用）
            #   触发本地限流        -> 429（请求过多）
            #   其余大模型错误       -> 502（网关错误）
            if e.code in ("LLM_NOT_CONFIGURED", "LLM_CIRCUIT_OPEN"):
                status = 503
    ...
    def _handle_save_user_config(self):
        ...
        # 字段白名单 + 长度校验
        allowed = {"api_url", "model_name", "api_key",
                   "semester_start", "total_weeks", "extra_settings"}
        data = {k: v for k, v in body.items() if k in allowed}
        if "api_url" in data and len(str(data["api_url"] or "")) > 512:
            self._send_json({"error": "API URL 过长", "code": "VALIDATION_ERROR"}, status=400)
            return
        if "model_name" in data and len(str(data["model_name"] or "")) > 128:
            self._send_json({"error": "模型名过长", "code": "VALIDATION_ERROR"}, status=400)
            return
...
def main():
    ...
    try:
        courses = parse_mock_courses()
        print(" 已加载 %d 门课程 (test_data.js)" % len(courses))
    except Exception as e:
        print(" [警告] 数据加载失败: %s" % e)
    ...
    elif cfg.has_api_key:
        print(" 大模型: %s @ %s" % (cfg.model, cfg.base_url))
    else:
        print(" 大模型: 未配置 API Key（/api/ai/parse 将返回 503；"
              "请在 .env 设置 LLM_API_KEY 或设 mock=true 测试）")

    # 打印用户配置管理模块状态
    if _USER_CONFIG_ENABLED:
        print(" 用户配置: 已启用 (MySQL 已连接)")
        print("           详见 README_USER_CONFIG.md")
    else:
        print(" 用户配置: 未启用 (MySQL 不可用)")
        print("           配置 .env 中的 DB_* 变量并 pip install PyMySQL cryptography 后启用")
    print("")
