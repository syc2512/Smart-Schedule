...
职责：
  1. 用户密码哈希与校验（PBKDF2-HMAC-SHA256 + 用户级 salt）
  2. Session 票据签发与校验（HMAC-SHA256 签名 Cookie）
...
# 统一日志系统（用于启动告警；模块自检仍用 print）
try:
    from logger import get_logger as _get_logger
    _log = _get_logger("crypto")
except Exception:
    class _FallbackLog:
        def warning(self, *a, **k):
            try:
                sys.stderr.write("[crypto] " + (a[0] % a[1:] if len(a) > 1 else str(a[0])) + "\n")
            except Exception:
                pass
    _log = _FallbackLog()
...
class CryptoService:
    ...
    def __init__(self, secret_key: Optional[str] = None):
        ...
        if not key:
            ...
            _log.warning("未设置 TIMETABLE_SECRET_KEY，使用临时密钥（仅开发用）")
...
if __name__ == "__main__":
    """本地直接运行 crypto.py 时的基本自检：密码哈希 + Session 票据。"""
    svc = CryptoService()
    h, s = svc.hash_password("test123")
    assert svc.verify_password("test123", h, s), "密码校验应成功"
    assert not svc.verify_password("wrong", h, s), "错误密码应校验失败"
    ticket = svc.create_session_ticket(user_id=1)
    assert svc.verify_session_ticket(ticket) == 1, "Session 票据应可解析"
    assert svc.verify_session_ticket(ticket + ".x") is None, "篡改的票据应校验失败"
    print("[crypto] 自检通过：密码哈希 / Session 票据")
