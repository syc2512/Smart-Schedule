...
class AuthService:
    ...
    def register(self, username: str, password: str,
        ...
        try:
            ...
            if e.code == "DB_DUPLICATE_USERNAME":
                _log.warning("注册失败-用户名已存在 | username=%s", username)
                raise AuthError("用户名已被占用", code="AUTH_USERNAME_TAKEN")
            _log.error("注册失败 | username=%s | %s", username, e.message)
        ...
        try:
            ...
            pass  # 非关键步骤
        _log.info("用户注册并登录 | id=%d username=%s", user_id, username)
        return AuthResult(
    ...
    def authenticate(self, username: str, password: str) -> AuthResult:
        ...
        if cred is None:
            ...
            _log.warning("登录失败-用户不存在 | username=%s", username)
            raise AuthError("用户名或密码错误", code="AUTH_INVALID_CREDENTIALS")
        if not cred.get("is_active"):
            _log.warning("登录失败-账号被禁用 | username=%s", username)
        ...
        if not ok:
            _log.warning("登录失败-密码错误 | username=%s", username)
            raise AuthError("用户名或密码错误", code="AUTH_INVALID_CREDENTIALS")
        ...
        _log.info("用户登录成功 | id=%d username=%s", cred["id"], username)
        return AuthResult(
